...
const ProfilePanel = ({ panelOpen, onClose }) => {
  ...
  const handleLogout = () => {
    ...
    axios
      ...
      .then((response) => {
        console.log(
          "Frontend axios handling LOGOUT and this is:",
          response.data
        );
        ...
        if (!response.data.success) {
          ...
          navigate("/login");
          if (onClose) onClose(); // Close panel after logout
        }
      })
      .catch((err) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black bg-opacity-50 backdrop-blur-sm">
      ...
      >
        ...
        {/* Profile Info */}
        <div
          className={`flex flex-col items-center py-6 space-y-2 border-b transition-all 
          ${darkMode ? "border-gray-900" : "border-gray-200"}`}
        >
          <motion.img
        ...
        <div className="flex flex-col flex-grow p-4 space-y-4">
          <motion.div
            ...
            className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 cursor-pointer transition"
          >
            <UserCircle size={22} />
        ...
        >
          <motion.button
            ...
            className={`flex items-center space-x-2 w-full p-3 rounded-lg text-white transition-all 
              ${
                darkMode
